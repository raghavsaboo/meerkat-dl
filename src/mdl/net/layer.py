...
from typing import List, Any, Dict, Any, Union
    ...
    def aggregate_parameters(self, as_list: bool = False) -> List[Parameter] | Dict[str, Parameter]:
        ...
        for name, value in self.__dict__.items():
            if isinstance(value, Parameter):
                ...
                for sub_name, sub_param in operation_params.items():
...
class Sequence(ABC):
    ...
    def forward(self, input_tensor: List[Union[Tensor, Parameter]]) -> Union[Tensor, Parameter]:
    ...
    def aggregate_parameters(self, as_list: bool = False) -> List[Parameter] | Dict[str, Parameter]:
        ...
        for index, layer in enumerate(self.layers):
            ...
            for sub_name, sub_param in layer_parameters.items():
                parameters[f"{name}.{sub_name}"] = sub_param
...
class Module(ABC):
    ...
    def aggregate_parameters(self, as_list: bool = False):
        parameters = dict()
        
        for name, value in self.__item__.dict():
            if isinstance(value, Parameter):
                parameters[name] = value
            elif isinstance(value, (Module, Layer, Sequence, ParameterOperation)):
                sub_params = value.aggregate_parameters(as_list=False)
