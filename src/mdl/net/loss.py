...
class CrossEntropyLoss(Loss):
    def forward(self, outputs, targets):
        ...
        log_probs = -(probs[range(self.batch_size.data), targets.data.astype(int)]).log()
...
class HingeLoss(Loss):
    def forward(self, outputs, targets):
        ...
        correct_class_scores = outputs[range(num_samples), targets.data.astype(int)]
        margins = ((outputs - correct_class_scores.reshape(num_samples, 1) + 1.0)).max(Tensor(0.0))
...
class KLDivergenceLoss(Loss):
    def forward(self, outputs, targets):
        ...
        kl_div = (targets_probs * (targets_probs / outputs_probs).log()).sum(axis=1)
        loss = kl_div.mean()
        return loss
...
class DiceLoss(Loss):
    def forward(self, outputs, targets):
        ...
        dice_coeff = (Tensor(2.0) * intersection + smooth) / (outputs.sum() + targets.sum() + smooth)
